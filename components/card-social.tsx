...
export default function CardSocial({ icon, iconHover, title, text, url, buttonTitle, color, className, buttonClassName }: ICardSocial) {
	return (
		<ExternalLink href={url} className="block select-none">
			<Card className={`group/hover flex flex-col items-center text-center p-6 border-2 transition-all duration-300 shadow-lg hover:shadow-xl h-full group${className ? ` ${className}` : ""}`}>
				<CardContent className="flex flex-col items-center h-full space-y-4 pt-6">
					...
					<div className="h-full flex items-end">
						<div
							className={`w-full flex items-center justify-center group-hover/hover:rotate-cta group-hover/hover:shadow-sm whitespace-nowrap rounded-md text-sm font-medium transition-all duration-300 h-10 px-6 border ${color}${buttonClassName ? ` ${buttonClassName}` : ""}`}
						>
							{buttonTitle}
						</div>
					</div>
